...
- Use friendly illustrated visuals for the main story beats, with the source-provided poster preferred when it materially supports the lesson and the user has access to it. Keep the visual treatment consistent with the established poster-led deck; use generated or inline/vector artwork only when a source visual is unavailable or the approved outline calls for a small supporting illustration. Keep prompts age-appropriate and localize any generated assets.
- Make the deck feel child-friendly through expressive image crops, short headlines, colorful cards, large question buttons, and reveal bubbles—not by crowding the screen with extra cartoon overlays. Use speech/thought-style reveal bubbles only when they clarify a source-supported line or an explicitly labeled child-friendly wondering question; do not invent biblical dialogue.
